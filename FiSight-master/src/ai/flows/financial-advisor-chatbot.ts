...
const financialAdvisorPrompt = ai.definePrompt({
  ...
  prompt: async (input) => {
    ...
    const totalDebt = financialData.debts.reduce((sum: number, debt: any) => sum + debt.amount, 0);
    const totalInvestments = financialData.investments.reduce((sum: number, inv: any) => sum + inv.value, 0);
