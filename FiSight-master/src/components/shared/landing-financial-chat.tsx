...
import { User, CornerDownLeft, Sparkles, Brain, LogIn, ArrowRight } from 'lucide-react';
        ...
        content: (
          <Card className="border-primary/20 bg-gradient-to-r from-primary/5 to-blue-50">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2">
                <Button asChild className="w-full" suppressHydrationWarning={true}>
            ...
            <Card className="border-blue-200 bg-blue-50/50">
              <CardContent className="p-3">
                ...
                {newResponseCount >= MAX_FREE_RESPONSES - 1 && (
                  <Button asChild size="sm" className="w-full mt-2" suppressHydrationWarning={true}>
                    <Link href="/login" target="_blank">
  ...
  return (
    <ClientOnly>
      ...
      <SheetTrigger asChild>
        <Button
          className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90"
          size="icon"
          suppressHydrationWarning={true}
        >
    ...
    <SheetContent className="flex flex-col w-full sm:max-w-lg">
      ...
      </SheetHeader>        <div className="flex-1 flex flex-col space-y-4 min-h-0">
          <ScrollArea className="flex-1 pr-4 max-h-[60vh]">
            <div className="space-y-4 pb-4">
              {messages.map((message) => (
                <div key={message.id} className={`flex items-start gap-3 ${message.role === 'user' ? 'justify-end' : ''}`}>
                  {message.role === 'advisor' && (
                    <Avatar className="h-8 w-8 border bg-gradient-to-r from-primary to-accent">
          ...
          <div className="space-y-3">
            ...
            <form onSubmit={handleSendMessage} className="flex gap-2">
              <Input
                ...
                placeholder={responseCount >= MAX_FREE_RESPONSES ? t('nav.getStarted') : t('chat.placeholder')}
